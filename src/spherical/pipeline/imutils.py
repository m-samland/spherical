...
def _rotate_fft(array, alpha, pad=4, x1=0, x2=0, y1=0, y2=0, cval=0):
    ...
    pad_x = fft.ifft((fft.fft(pad_frame, axis=0, overwrite_x=True).T *
    ...
    pad_xyx = fft.ifft((fft.fft(pad_xy, axis=0, overwrite_x=True).T *
                        np.exp(a*((M*N).T*X).T)).T, axis=0, overwrite_x=True)
...
def profile(img, ptype='mean', step=1, mask=None, center=None, rmax=0, clip=True, exact=False):
    ...
    if step == 1:
        ...
        if ptype == 'mean':
            ...
            raise ValueError(
                'Unknown statistics ptype = {0}. Allowed values are mean, std, var, median, min and max'.format(ptype))
    else:
