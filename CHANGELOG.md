...
### ✨ Added
- **Gaia stellar parameters as TRAP defaults** – TRAP detection now auto-populates the host star's effective temperature, surface gravity, and metallicity for template matching from the `GAIA_TEFF` / `GAIA_LOGG` / `GAIA_MH` columns of the observation table, per observation. NaN or missing columns fall back to the configured (or default) `StellarParameters` values. Controlled by `trap_config.detection.use_gaia_stellar_parameters` (default `True`); set it to `False` to keep a fixed value for all targets ([@m-samland](https://github.com/m-samland)).
